...
def main(argv):
    parser = argparse.ArgumentParser(prog="train_vitasbench_101.py")
    ...
    setproctitle.setproctitle("python train_vitasbench_101.py config: {}; train_dir: {}; cwd: {}"\
    ...
    arch_embedder_type = cfg["arch_network_cfg"]["arch_embedder_type"]
    ...
    if arch_embedder_type == "gp":
        train_X = [data[0] for data in train_data]
        train_Y = [data[1] for data in train_data]
        valid_X = [data[0] for data in valid_data]
        valid_Y = [data[1] for data in valid_data]
        train_dataset = (train_X, train_Y)
        valid_dataset = (valid_X, valid_Y)
        pred = model.arch_embedder(train_dataset, valid_dataset)
    
        corr = stats.kendalltau(valid_Y, pred).correlation
        patk = p_at_tb_k(valid_Y, pred)
        natk = minmax_n_at_k(valid_Y, pred)

        patk = [x[2] for x in patk]
        natk = [x[1] for x in natk]
        logging.info("Valid: Epoch {:3d}: kendall tau {:.4f}".format(1, corr))
        logging.info(patk)
        logging.info(natk)
        return
